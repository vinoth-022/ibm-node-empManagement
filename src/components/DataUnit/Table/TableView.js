...
export default function TableView({tableData, setTableData, setSelectedFlatRows, setIsOneRowSelected, setIsRowSelected, updateTable}) {
    ...
    const {                                             //these functions are returned from useTable
        getTableProps,
        getTableBodyProps,
        headerGroups,
        prepareRow,
    ...
    return (
        <div className='tableView' align="center">
            <TableContainer className='tableContainer'>   {/* tableContaier from materialUI*/}

                {/* Table from react*/}
                <Table stickyHeader className='table' aria-label="sticky table" {...getTableProps()}> 

                    {/* TableHead from react, headGrp is a list we loop, for each loop we return table row nad destructure headergrp props*/}
                    <TableHead>
